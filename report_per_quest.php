...
<?php 
	...
	if(empty($quest) || empty($option) || ($quest>5 || $quest <0) || ($option>5 || $option <0))
			redirect_to("./?Report");	

	$message = "";
	$getquery = "./?Report=".$_GET["Report"];
	...
	$total_count = Response::count_all_by_quest_option($form->id,$quest,$option,$startTime, $endTime);//26 for all responses

	$pagination = new Pagination($page, $per_page, $total_count);

	$responses = Response::find_all_by_quest_option_paginated($form->id,$quest,$option,$per_page,$pagination->offset(),$startTime,$endTime);//26 for all responses
	if($total_count ==0 )$message = $message." "."There are not any notifications.";	
	

?>
<div class="reports-per-question" style="font-size:1.1em;font-weight:bold;">
	<div style="background-color:#f7bf24;width: 100%;text-align:center;border-bottom: 5px solid #CEE1E5;height:38px;">
		<span style="color:#fff;vertical-align:middle;">FILTER YOUR RESPONSES</span>
		<div class="datePicker"style="display: inline-block;">
			<form action method="get">
				...
				<input type="button" class="css3button" value="ALL"  onclick="window.location.href = './?Report=<?php echo $_GET["Report"]; ?>';">
			</form>
		</div>
	</div>
	<div style="height: 30px;background-color: #29859a;color: #fff;border-bottom: 5px solid #CEE1E5;">
		<p style="padding: 6px 10px;">Following people responds to question <?php echo $quest; ?> with option "<?php echo $opt; ?>" :</p>
	</div>
	<table style="width: 100%;font-weight: normal;">
		<?php foreach($responses as $resp){?>
			<tr style="cursor:pointer;" id="resp<?php echo $resp->id; ?>" class="<?php if($resp->viewed==0)echo "new";?>"><td><?php echo $resp->name; ?>(Click to view feedback)</td><td title="<?php echo strftime("%B %d, %Y at %I:%M %p", $resp->time);?>"><?php echo nicetime($resp->time);?></td></tr>
		<?php }?>
	</table>
...
<script type="text/javascript">
	jQuery(document).ready(function(){
        ...
        $('.reports-per-question table tr').click(function(){
