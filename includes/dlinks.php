...
class Dlink {
  	...
  	protected static $table_name="dlinks";
	protected static $db_fields=array('id', 'hash', 'user_id','count');
	public $id;
	public $hash;
	public $user_id;
	public $count;
  ...
  // "new" is a reserved word so we use "make" (or "build")
	public static function make($user_id,$count) 
	{
	    if(!empty($user_id))
	    {
				$link = new Dlink();
	  ...
	  public static function find_by_hash($hash) {
	  	$s = "SELECT * FROM ".self::$table_name." WHERE hash='{$hash}' LIMIT 1";
